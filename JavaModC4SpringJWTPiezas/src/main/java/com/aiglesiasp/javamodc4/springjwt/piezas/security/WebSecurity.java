...
import static com.aiglesiasp.javamodc4.springjwt.piezas.security.Constants.LOGIN_URL;
...
public class WebSecurity extends WebSecurityConfiguration {
	...
	protected void configure(HttpSecurity httpSecurity) throws Exception {
		...
		httpSecurity.sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and().cors().and()
				.csrf().disable().authorizeRequests().requestMatchers(HttpMethod.POST, LOGIN_URL).permitAll().anyRequest()
